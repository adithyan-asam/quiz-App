package com.example.quizzedapp.adapters
...
import com.example.quizzedapp.activities.Authentication
import com.example.quizzedapp.models.User
...
class RegisterAdapter(private val context: Context, private val onSuccess: () -> Unit): Authentication() {
    ...
    fun createUserWithEmailAndPassword(email: String, password: String, confirmPassword: String) {
        ...
        if (!email.endsWith("@student.iiests.ac.in")) {
            Toast.makeText(context, "Only @student.iiests.ac.in emails are allowed", Toast.LENGTH_SHORT).show()
            return
        }
        user = User().apply {
            this.email = email
